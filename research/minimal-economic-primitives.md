...
| Exact monetary value representation | `NORMALIZED_CANDIDATE` | TH-INTEROP-24 shows that explicit asset identity + arbitrary-precision signed coefficient + explicit base-10 scale can preserve lossless normalized magnitudes across UBL decimal, UCP/Stripe minor-unit, and x402 token-atomic shapes while retaining source representation. Cross-basis exact-quantity equivalence remains unearned without grounded scale metadata. This is a representation technique, not a Money business object. | Re-test inside an authoritative operational quote/estimate mapping; reject or narrow if price/asset/scale/rounding semantics cannot remain auditable. |
| Rich Offer / quote | `NORMALIZED_CANDIDATE` | TH-INTEROP-24 resolves the binary-number representation problem only at a research level. Line-item meaning, taxes, terms, lifecycle, price basis, authority, and provider-specific semantics remain unproven. | Prove reversible-enough mapping across at least one protocol representation and one authoritative operational quote/estimate source before considering profile promotion. |
...
TH-INTEROP-23 is intentionally pinned to UBL 2.4, which contains Request For Quotation, Quotation, Order and related mature procurement semantics. Its Ordering process explicitly creates a contractual obligation between buyer and seller, which is one reason an AIP Bind cannot be silently renamed into the same universal Commitment transition. OASIS has since published UBL 2.5 as an OASIS Standard on 2026-08-12, superseding 2.4; that newer release is prior-art context, not retroactive conformance evidence for the 2.4-shaped falsifier fixtures.
...
The Requirement falsifier, the Request -> Offer -> Commitment boundary falsifier, and the bounded exact-money representation falsifier are complete at their current evidence level. The items below are a research backlog of remaining uncertain boundaries, **not the repository-wide execution order**. Current sequencing is defined only by the `STATUS.md` **Next gate** section; it now places the authoritative operational quote/estimate experiment next.

High-value later falsification targets include:

1. **Operational quote falsifier** — use the TH-INTEROP-24 exact-value representation candidate while mapping one authoritative quote/estimate through an independent representation; test round-trip/auditability against the earned thin OfferResponseObservation without assuming taxes/terms/lifecycle normalize.
