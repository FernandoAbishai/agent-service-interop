...
The current gate is to test the `OfferResponseObservation` hypothesis plus the exact-value representation candidate against an **authoritative operational quote/estimate source**. Direct ServiceTitan Integration Environment evidence remains strategically preferred because it would also advance the second-live-origin track, but no ServiceTitan credentials/tenant access are available in the current development environment. TH-INTEROP-25 therefore prepares the approved read-only Jobber Quote fallback. Deterministic code does **not** close this gate: authoritative Jobber Quote evidence remains pending until the opt-in live test succeeds against a test-account Quote. Jobber's documented `Quote.request` is treated only as a source-native association, not yet as proof that the Quote directionally responds to the Request; presence or absence must not be repaired from Job/status/booking facts. Rich quote terms, lifecycle, taxes, line-item semantics, and authority remain unearned until that live mapping is evaluated.
